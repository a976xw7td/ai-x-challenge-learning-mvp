...
export default function KnowledgePage() {
  ...
  const types = Array.from(new Set(knowledgeItems.map((k) => k.type)));
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="搜索知识库... (FAQ、教材、Prompt、最佳实践)"
            value={search}
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg border ${typeColors[item.type] || "bg-gray-50 text-gray-600 border-gray-200"}`}
                >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`inline-flex items-center rounded border px-1.5 py-0.5 text-xs font-medium ${typeColors[item.type] || "bg-gray-50 text-gray-600 border-gray-200"}`}
                    >
                      {item.type}
                    </span>
                  </div>
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-gray-400">
          ...
          <p className="text-xs">尝试更换搜索关键词或筛选条件</p>
        </div>
      )}
    </div>
  );
